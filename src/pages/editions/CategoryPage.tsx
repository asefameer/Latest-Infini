...
const CategoryPage = () => {
  ...
  const rawProducts = categorySlug ? allProducts.filter(p => p.category === categorySlug) : allProducts;
  ...
  return (
    <>
      <SEOHead title={category?.name || 'All Products'} description={category?.description || 'Browse the complete Infinity collection.'} canonical={`/editions/c/${categorySlug}`} />
      <div className="container mx-auto px-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setShowFilters(!showFilters)} className="md:hidden flex items-center gap-2 rounded-full border border-border/40 px-4 py-2 text-sm hover:bg-muted/50 transition-colors">
              <SlidersHorizontal className="w-4 h-4" /> Filters
            </button>
            <div className="relative">
              <select value={sort} onChange={e => setSort(e.target.value as SortOption)} className="appearance-none rounded-full border border-border/40 bg-transparent px-5 py-2 pr-9 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30">
          ...
          <aside className={`w-56 flex-shrink-0 space-y-6 ${showFilters ? 'block' : 'hidden md:block'}`}>
            <div>
              ...
              <div className="flex items-center gap-2 text-sm">
                ...
                <input type="range" min={0} max={15000} step={500} value={priceRange[1]} onChange={e => setPriceRange([priceRange[0], Number(e.target.value)])} className="flex-1 accent-primary" />
