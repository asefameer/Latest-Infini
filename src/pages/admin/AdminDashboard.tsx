...
import { useProducts, useEvents } from '@/services/api/hooks';
...
const liveEvents: Omit<ActivityItem, 'id' | 'isNew'>[] = [
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setLiveIdx(prev => {
        ...
        const newActivity: ActivityItem = { ...event, id: `live-${Date.now()}`, isNew: true };
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {stats.map(s => (
          <Link key={s.label} to={s.href} className="bg-card border border-border rounded-xl p-5 hover:border-primary/30 transition-colors group">
        ...
        {trafficMetrics.map(m => (
          <div key={m.label} className="bg-card border border-border rounded-xl p-5">
            <div className="flex items-center justify-between mb-3">
              <div className={cn('w-9 h-9 rounded-lg flex items-center justify-center', m.color)}><m.icon className="w-4 h-4" /></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        <div className="lg:col-span-2 bg-card border border-border rounded-xl p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="flex items-center gap-1.5 text-xs text-primary"><span className="w-2 h-2 rounded-full bg-primary animate-pulse" />Live</span>
          </div>
          <div className="space-y-1 max-h-[420px] overflow-y-auto pr-1">
            {activities.map(a => {
              ...
              return (
                <div key={a.id} className={cn('flex items-start gap-3 p-3 rounded-lg transition-all duration-500', a.isNew ? 'bg-primary/5 border border-primary/20' : 'hover:bg-muted/30')}>
                  <div className={cn('w-8 h-8 rounded-lg flex items-center justify-center shrink-0 mt-0.5', config.color)}><Icon className="w-4 h-4" /></div>
        ...
        <div className="bg-card border border-border rounded-xl p-6 flex flex-col">
          ...
          <div className="space-y-2 flex-1">
            {[
              { to: '/admin/products/new', label: '+ Add Product', style: 'bg-primary/10 text-primary hover:bg-primary/20' },
              { to: '/admin/events/new', label: '+ Add Event', style: 'bg-secondary/10 text-secondary hover:bg-secondary/20' },
              { to: '/admin/banners', label: 'Manage Banners', style: 'bg-muted text-muted-foreground hover:bg-muted/80' },
              { to: '/admin/crm/customers', label: 'View Customers', style: 'bg-muted text-muted-foreground hover:bg-muted/80' },
              { to: '/admin/crm/analytics', label: 'Analytics', style: 'bg-muted text-muted-foreground hover:bg-muted/80' },
            ].map(a => (
              <Link key={a.to} to={a.to} className={`flex items-center justify-between px-4 py-3 text-sm rounded-lg transition-colors group ${a.style}`}>
                <span>{a.label}</span>
